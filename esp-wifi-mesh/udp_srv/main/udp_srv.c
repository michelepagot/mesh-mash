...
void udp_server_task(void *arg)
{
  ...
  while (1) {
    ...
    if (!esp_mesh_is_root()) {
      ESP_LOGE(MESH_TAG, "Server task have ONLY to run on <ROOT>");
      vTaskDelete(NULL);
      continue;
    }
    ESP_LOGI(MESH_TAG, ">>>> udp_server_task <<<<");
    ...
    while (1) {
      ESP_LOGI(MESH_TAG, "Waiting for data");
      ...
      if (len < 0) { // Error occurred during receiving
        ESP_LOGE(MESH_TAG, "recvfrom failed: errno %d", errno);
        break;
      }
      else { // Data received
        ESP_LOGI(MESH_TAG, "Received %d bytes", len);
      }
    }
    vTaskDelay(1 * 1000 / portTICK_PERIOD_MS);
  }
...
esp_err_t udp_server_start(void)
{
    ...
    if (!is_server_started) {
        ...
        xTaskCreate(udp_server_task, "SRV", 4096, NULL, 5, NULL);
    }
    return ESP_OK;
}
